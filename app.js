...
const weeklySchedule = [
  ...
  { day:'Venerdì', workoutIndex:4, type:'main', label:'Upper C', title:'Upper C — petto alto + spalle/braccia', note:'Petto alto, spalle e braccia.' },
  { day:'Sabato', workoutIndex:null, type:'optional', label:'Opzionale', title:'Vacanza rapida / camminata / recupero', note:'Usa Vacanza rapida se vuoi muoverti, altrimenti camminata e mobilità.' },
  { day:'Domenica', workoutIndex:null, type:'rest', label:'Riposo', title:'Riposo, camminata o mobilità', note:'Recupero attivo. Puoi anche fare una seduta leggera se hai saltato un giorno.' }
];

function recommendedWorkoutIndex(){
...
function todayScheduleItem(){
  ...
  return weeklySchedule[idx];
}
function loadSelectedDay(){
  const saved = Number(localStorage.getItem(STORE.selectedDay));
  return Number.isInteger(saved) && saved >= 0 && saved < workouts.length ? saved : recommendedWorkoutIndex();
}
function saveSelectedDay(){
  localStorage.setItem(STORE.selectedDay,String(state.selectedDay));
}
...
function bindNavigation(){
  ...
  $('#dayPicker').addEventListener('change',e=>{state.selectedDay=Number(e.target.value); saveSelectedDay(); renderToday(); renderCalendar();});
...
function renderAll(){
  ...
  renderToday(); renderCalendar(); renderWorkouts(); renderVacation(); renderExtra(); renderLibrary(); renderDiet(); renderCarbTool(); renderProgress();
}
function renderToday(){
  const day=workouts[state.selectedDay];
  const today = todayScheduleItem();
  $('#todayTitle').textContent=day.name;
  const badge = $('#todayAutoBadge');
  if(badge) badge.textContent = `Oggi: ${today.day} · ${today.workoutIndex !== null ? today.label : today.label}`;
...
function renderCalendar(){
  ...
  wrap.innerHTML = weeklySchedule.map((item,idx)=>{
    const isToday = idx === todayIdx;
    const selected = item.workoutIndex === state.selectedDay && item.workoutIndex !== null;
    const workout = item.workoutIndex !== null ? workouts[item.workoutIndex] : null;
    return `<article class="calendar-card ${isToday?'today':''} ${selected?'selected':''}">
      <div class="calendar-top">
        <span class="calendar-day">${item.day}</span>
        <span class="calendar-pill ${item.type}">${item.label}</span>
      </div>
      <h4>${item.title}</h4>
      <p>${item.note}</p>
      ${workout ? `<div class="calendar-actions">
        <button class="primary small calendar-open" data-index="${item.workoutIndex}">Apri scheda</button>
        <button class="ghost small calendar-set" data-index="${item.workoutIndex}">Imposta in Home</button>
      </div>` : `<div class="calendar-actions">
        <button class="ghost small quick-nav" data-targettab="vacation">Vai a Vacanza</button>
      </div>`}
    </article>`;
  }).join('');
  $$('.calendar-open',wrap).forEach(btn=>btn.addEventListener('click',()=>{
    state.selectedDay=Number(btn.dataset.index); saveSelectedDay(); $('#dayPicker').value=String(state.selectedDay); renderToday(); renderCalendar(); showTab('workouts');
  }));
  $$('.calendar-set',wrap).forEach(btn=>btn.addEventListener('click',()=>{
    state.selectedDay=Number(btn.dataset.index); saveSelectedDay(); $('#dayPicker').value=String(state.selectedDay); renderToday(); renderCalendar(); toast('Allenamento impostato in Home.');
  }));
  $$('.quick-nav',wrap).forEach(btn=>btn.addEventListener('click',()=>showTab(btn.dataset.targettab)));
}
...
function startGuidedRest(dayId,exId,e){
  ...
  $('#guidedTitle').textContent=e.name;
  $('#guidedEyebrow').textContent='Recupero tra le serie';
  $('#guidedModal').showModal();
  updateGuidedRestScreen(dayId,exId,e);
  state.guidedRestId=setInterval(()=>updateGuidedRestScreen(dayId,exId,e),250);
}
function updateGuidedRestScreen(dayId,exId,e){
  ...
  $('#guidedContent').innerHTML=`
    <div class="guided-box rest-box">
      ...
      <div class="guided-count rest-count">${m}:${s}</div>
      <div class="exercise-actions">
        <button class="guided-primary" onclick="skipGuidedRest()">Salta recupero</button>
      </div>
    </div>`;
  if(remaining<=0){
    clearGuidedRest();
    beep(); vibrate(); showDoneNotification();
    updateGuidedContent();
  }
}
function clearGuidedRest(){
  if(state.guidedRestId){ clearInterval(state.guidedRestId); state.guidedRestId=null; }
  state.guidedRestEnd=null;
}
function skipGuidedRest(){
  clearGuidedRest();
  beep();
  updateGuidedContent();
}
function finishGuided(){ clearGuidedRest(); state.guided=null; $('#guidedModal').close(); }
...
function beep(){
  try{
    ...
    const volume = Math.min(1,Math.max(.1,state.soundVolume || .95));
    const playTone = (time,freq,duration,type='sine')=>{
    ...
    if(state.soundType==='classic'){
      [0,.18,.36].forEach(o=>playTone(now+o,880,.13,'sine'));
    }else if(state.soundType==='strong'){
      [0,.22,.44,.66].forEach(o=>playTone(now+o,1040,.18,'square'));
    }else if(state.soundType==='bell'){
      [0,.25,.50].forEach(o=>{ playTone(now+o,1320,.20,'triangle'); playTone(now+o+.04,1760,.16,'sine'); });
    }else if(state.soundType==='long'){
      playTone(now,740,.75,'sawtooth'); playTone(now+.82,980,.45,'square');
    }else{
      [0,.18,.36].forEach(o=>playTone(now+o,880,.13,'sine'));
    }
  }catch{}
...
window.skipGuidedRest=skipGuidedRest; window.openGuided=openGuided; window.completeGuidedSet=completeGuidedSet; window.finishGuided=finishGuided; window.openPoster=openPoster; window.toast=toast;
